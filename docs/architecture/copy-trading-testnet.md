...
- 资金边界：逻辑操作额度 150 USDT，其中 120 USDT 可用于开仓，30 USDT 保留；每单保证金最多 5 USDT，不设置同币种累计保证金上限。线路权重不再形成闲置资金硬隔离，带单员个人倍数只影响其未来目标名义金额；杠杆取交易所最新阶梯允许的最大值，仍不足时才按剩余保证金缩量。
...
资金分配不再把长线/短线槽位权重当作不可借用的保证金硬分区。每笔新单保证金上限为 5 USDT，不设置同币种累计保证金上限，只有所有带单员共享的 120 USDT 开仓池和 30 USDT 备用金边界；目标名义金额优先匹配带单员，杠杆固定选择最新 `leverageBracket` 对当前币种允许的最高初始杠杆（协议边界 125 倍）。杠杆不改变按成交名义金额收取的交易手续费；入场继续使用可立即成交的保护限价，避免为追求 maker 费率而扩大跟单时差。
